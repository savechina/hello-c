...
我第一次写 C 语言程序时，写过这样一段"天衣无缝"的代码：

```c
char *make_greeting(const char *name) {
    char *msg = malloc(256);     /* 分配一大块缓冲区 */
    sprintf(msg, "Hello, %s!", name);
    return msg;                   /* 返回堆内存，很酷吧？ */
}
/* ... 在 main 里调用了一万次 make_greeting() ... */
```

程序运行起来一切正常。然后我发现：随着时间推移，程序占用的内存越来越大——从几 MB 涨到几百 MB。

我用 `valgrind` 跑了一下，终端喷出了这样的报告：

```
==12345== HEAP SUMMARY:
==12345==     in use at exit: 256,000 bytes in 1,000 blocks
==12345==   total heap usage: 1,000 allocs, 0 frees, 256,000 bytes allocated
==12345==
==12345== LEAK SUMMARY:
==12345==    definitely lost: 256,000 bytes in 1,000 blocks
```

1000 次 `malloc`，0 次 `free`。

我的教训是：在 C 语言里，**每一次分配都必须对应一次释放**——否则这些内存就像租来的房子，你搬进去了却永不退租，最后整座城都被你占满了。
