...
我第一次接触 C 语言的变量时，觉得它就像一个贴了标签的储物柜。每个储物柜（变量）都有一个名字（变量名），里面可以放一种特定类型的东西（数据类型）。你一旦决定这个柜子放衣服（`int`），就不能突然往里面塞水（`double`）——除非你用一个特殊的容器装起来（类型转换）。

我发现 C 语言的变量和 Python、JavaScript 很不同：它要求你在使用之前就声明类型。起初我觉得这是多余的约束，但写了几个程序后，我发现这种「提前声明」让编译器能帮我提前发现很多错误。就像去超市前写好购物清单，比到了超市再临时想效率高得多。

我的理解是：**变量是程序的记忆单元，表达式是程序的计算肌肉**。二者结合，就是 C 语言最基础的能力。
