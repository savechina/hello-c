...
大三那年，我写了一个图像处理程序，编译通过，运行也不崩溃，但输出的图片全部是黑的。没有任何错误信息，没有 crash，没有 warning——就是结果不对。

我在黑暗中摸索了两天。没有 `errno` 检查，没有 `assert`，没有打印中间变量的日志。我甚至连输入数据是否正确都不知道。

最后我加了一行 `fprintf(stderr, "DEBUG: image_data = %p, size = %d\n", data, size);`，才发现数据指针是 NULL。两个昼夜的 debug，一行日志搞定。

从那以后，我养成了两个习惯：
1. **每个可能失败的函数调用都检查错误**
2. **关键路径有 debug 日志**

本章我就带你掌握 C 语言中所有的调试和错误处理工具。
