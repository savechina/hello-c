...
我写第一个多文件 C 程序的那天，遇到了一个让我抓狂的错误。我把函数声明直接写在 `.c` 文件里，然后在另一个文件用 `extern` 引用。编译时没问题，链接时报了"重复定义"——因为我在两个地方都写了同一个函数的实现。

后来老师告诉我：**头文件只写声明，源文件才写实现**。而且同一个头文件不能被包含两次，否则会产生重复定义。这把我整懵了——`#include` 不就是"复制粘贴"吗？怎么还会重复？

从那以后，我花时间搞懂了 C 的完整编译流程：预处理器 → 编译器 → 汇编器 → 链接器。理解了每个阶段的职责后，头文件的作用、`static` 与 `extern` 的区别、以及为什么需要 include guard，全都豁然开朗了。

本章我就带你搞懂 C 的头文件与模块系统，让你的代码从"单个文件"走向"模块化编程"。
