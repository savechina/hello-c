...
我第一次碰到 `void*` 是在读一个 C 语言库的源码时。那个库提供了一个排序函数, 签名长这样：
...
我懵了 —— `void*` 是什么？它既不是 `int*` 也不是 `char*`, 它到底指向什么？后来我才知道, 这是 C 语言在没有模板和泛型的情况下, 实现"一种函数处理多种类型"的方案。

我的理解是：`void*` 就像是一个"万能快递盒"——你可以往里面塞任何类型的地址, 但取出东西时, 你必须自己记得里面装的是什么, 否则拆错了就可能数据错乱甚至程序崩溃。
