...
第一次学 C 语言字符串时，我以为它和 Python 一样简单。直到我写了这样一段代码：

```c
char name[5];
strcpy(name, "Hello World!");  /* 我想存 "Hello World!" 到 name 里 */
printf("%s\n", name);          /* 程序崩溃了 😱 */
```

我很困惑：为什么 Python 里 `name = "Hello World!"` 一行搞定，C 语言却直接崩溃？后来我明白了——**C 语言的字符串不是对象，而是一块需要你自己管理的内存**。你需要自己分配空间、自己加终止符 `\0`、自己检查边界。这就是为什么 C 的缓冲区溢出漏洞常年占据安全排行榜前列。

那天我发现：**在 C 语言中，字符串处理不是"写代码"，而是"管理内存"**。每一个 `\0`、每一个 `sizeof`、每一个 `strncpy` 的第三个参数，都是在和计算机的内存管理机制对话。
