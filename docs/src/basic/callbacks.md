...
函数指针的真正力量不在于「调用一个已知函数」，而在于——**把函数交给别人去调用**。这就是回调（Callback）——把函数指针作为参数传入另一个函数，由那个函数在适当的时机调用它。

我第一次接触到这个概念是在做 GUI 编程（在 C++ 里）——给按钮注册一个 `onClick` 回调。后来我回到 C 语言，发现 `qsort` 就是最著名的回调：你把比较函数传进去，`qsort` 来决定什么时候调用。

这一章我用 `qsort` 的例子作为引子，深入讲解回调函数的模式、`void*` 泛型数据传递、事件驱动模拟，以及用函数指针实现多态——这是 C 语言在面向对象之前最强大的抽象工具。
