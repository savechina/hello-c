...
我第一次写命令行程序时，以为只要拿到 `argv[1]` 就可以了：
...
第一次运行 `./hello.txt "Hello"` 没问题。但当我忘记传参数，只运行 `./hello` 时——**Segmentation Fault**。

我很困惑：Python 中 `sys.argv[1]` 越界会抛出 `IndexError`，C 语言却直接崩溃？后来我明白了——**C 语言不会替你检查边界，`argv[1]` 不存在时访问它就是越界，是未定义行为**。

再后来我学会了 I/O 重定向：

```bash
./program > output.txt        # 输出保存到文件
./program < input.txt         # 从文件读取输入
cat log.txt | grep "ERROR"    # 管道连接程序
```

那天我发现：**命令行 I/O 的精妙之处不在于程序本身，而在于程序与操作系统的协作**。一个写得好的命令行程序，不需要知道它的输入来自键盘还是文件，输出显示在终端还是被重定向到管道。这是"Unix 哲学"的核心——程序只做一件事，但通过 I/O 重定向可以无限组合。
