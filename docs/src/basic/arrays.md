...
在 C 语言中，当我第一次遇到"数组"时，我觉得它就像一个有多个抽屉的柜子——每个抽屉能装同一种东西，而且每个抽屉都有编号。

```c
int32_t scores[5];  // 5 个抽屉，编号 0~4
```

但很快我发现一个有趣的事实：C 语言**不检查**你有没有打开超出编号的抽屉！
...
printf("%d\n", data[3]);  // ❌ 越界！编译器没报错，但结果是个随机数
```

当时我完全困惑："明明数组里没有这个值，怎么还能打印出来？"

后来才明白：C 语言信任程序员——你告诉我数组大小，它就把那片内存给你，至于你有没有"越界"，**你自己负责**。这就是为什么 C 程序既高效又容易出错。

当我学会用 `sizeof` 算出数组长度，再小心控制索引边界后，我终于能可靠地操作数组了。
