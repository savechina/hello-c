...
我第一次接触位运算是在写一个嵌入式系统的配置寄存器。寄存器只有 32 位, 每个 bit 代表一个开关：bit 0 开/关 LED, bit 1 开/关蜂鸣器, bit 2 开/关电机…… 我需要在 C 代码中精确地单独设置某一位, 同时不能影响其他位。

这让我意识到位运算不是"冷门的数学游戏" —— 它是底层编程的**基本功**。从操作系统权限到网络协议, 从嵌入式寄存器到游戏引擎, 位运算无处不在。

我的理解是：位运算就是直接和硬件"对话" —— 通过 AND/OR/XOR 这些逻辑门操作, 精确控制每一个 bit。
