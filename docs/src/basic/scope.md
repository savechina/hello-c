...
我第一次写出"诡异 bug"的那天，写了一个看起来天衣无缝的函数：
...
我以为它会输出 `20`，但实际运行结果是 `20` 或者一个随机垃圾值——取决于编译器优化级别和运气。这就是**使用 dangling pointer（悬垂指针）**的典型症状：`result` 在 `get_max` 返回后就消失了，但指针还在指向那片已经释放的内存。

我调了一整个下午才发现这个错误。从那以后，我深刻理解了 C 语言的核心概念之一：**作用域与生命周期**。

变量不是永远存在的。它们在被创建时"出生"，在离开作用域时"死亡"。理解这一点，90% 的指针问题都可以避免。

本章我就带你彻底搞懂 C 语言的作用域与生命周期。
