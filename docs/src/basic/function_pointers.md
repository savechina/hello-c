...
函数指针是 C 语言中最强大但也最令人困惑的特性之一。我第一次见到 `int (*fp)(int, int)` 时，脑子里只有一个念头：「这他妈是什么语法？」

之前我学的指针都很直观——`int *p` 指向一个整数，`char *s` 指向一个字符串。但函数指针指向的不是数据，而是**代码**。它存储的是一个函数的入口地址，调用它等于跳到那个地址去执行。

让我带你一步步拆解这个让人头大的语法，从错误写法开始，到理解它们在内存中的样子，最后写出能实际使用的函数指针代码。
