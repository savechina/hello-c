...
在 C 语言中，当我第一次需要把"学生的姓名、年龄、成绩"打包在一起时，我意识到变量和数组远远不够——我需要一种能把不同类型数据捆绑起来的容器。
...
我第一次用 `struct` 是在大二的 C 语言课程作业里——做一个学生管理系统。

```c
// 一开始我用了三个独立的数组，管理起来极其痛苦
char names[100][32];
int  ages[100];
float scores[100];
// 删除一个学生时，要同时修改三个数组！稍不留神就错位。
```

后来老师教了 `struct`，我才恍然大悟——**结构体，就是把有关联的数据"装进同一个盒子"的工具**。就像把一个学生的姓名、年龄、成绩打包成一份档案，而不是散落在三个抽屉里。

学了结构体之后，我才真正理解了为什么要用 struct——它让代码从"一堆变量"变成了"有意义的实体"。
