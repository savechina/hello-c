...
我第一次遇到 struct padding 坑，是在做嵌入式开发的时候。
...
// 我以为 sizeof = 6，但实际是 12！
// 直接把 6 字节的数据拷贝到这个 struct 里，所有字段都错位了。
```

我当时完全懵了——为什么 1 + 4 + 1 ≠ 6？

后来查了资料才明白：**CPU 读取内存时有对齐要求**，`int32_t` 必须放在 4 的倍数地址上。编译器为了迎合 CPU，在字段之间悄悄塞了 padding 字节。

那次事故之后，我彻底搞懂了 struct 的内存布局，再也没踩过对齐的坑。
