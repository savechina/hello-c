...
我在写第一个 C 程序时，被一行代码坑了整整半小时：

```c
if (x = 5) { ... }
```

我本意是判断 `x 是否等于 5`，结果写成了赋值。因为 C 语言允许 `=` 表达式出现在条件中，程序不但没报错，反而每次都"真"地执行了。

这就是**运算符**的陷阱——它们看起来简单，但优先级、结合性、短路求值等特性稍不留神就会让你写出 bug。这一章我会带你彻底搞懂 C 语言的所有运算符。
