...
大一那年，我发现自己写代码有个坏习惯：把所有逻辑都塞进 `main()` 里。我的 `main()` 函数有 800 多行，像一锅大杂烩，改一处崩三处。

直到老师教了函数，我才恍然大悟——**函数，就是把复杂问题切分成小块的艺术**。就像做一道菜：切菜、炒菜、装盘，每一步都是独立的函数。

本章我就带你掌握 C 语言的函数，让你的代码从"面条代码"变成模块化程序。
