...
我第一次接触 C 语言的指针时，整个人是懵的。

之前用 Python 写代码，变量赋值就是 `a = 10`，`b = a`，简单明了。可到了 C 语言，突然出现了一堆 `*` 和 `&` 符号，像是某种神秘仪式的咒语：

```c
int a = 10;
int *p = &a;   // & 是取地址？那 * 又是什么？
*p = 20;       // 这行代码改变了 a 的值？为什么？
```

我发现自己陷入了一个困惑：`a` 和 `*p` 到底是什么关系？为什么修改 `*p` 会影响 `a`？

后来我才慢慢理解：**指针的本质就是「地址」**——它像一个快递单号，告诉你数据在内存仓库里的确切位置。`&a` 是问你 a 住在哪个地址，`*p` 是顺着地址 p 去找真正的数据。

我的理解是：指针不是魔鬼，它只是 C 语言给你的一把手术刀——锋利、强大，但用错了会割到自己。
