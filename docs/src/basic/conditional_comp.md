...
第一次需要在 macOS 和 Linux 上编译同一个 C 程序时，我傻眼了——`#include <mach/mach.h>` 在 Linux 上根本不存在。我以为要维护两份代码。

直到我发现了 `#ifdef __APPLE__` 的魔法：同一份源代码，编译器根据预定义的宏自动选择正确的路径。我兴奋地写道：
...
这是我第一次体会到 C 语言的可移植性——不是通过运行时的 if/else，而是通过**编译时**的条件选择。
