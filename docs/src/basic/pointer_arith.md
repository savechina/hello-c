...
我第一次尝试用指针遍历数组时，写了这样的代码：

```c
int32_t nums[4] = {10, 20, 30, 40};
int32_t *p = nums;
p = p + 4;           // 想跳到数组末尾
int32_t last = *p;   // ❌ 读到了垃圾值！
```

我当时很困惑："我明明只移动了 4 个位置，为什么读到的是乱码？"

后来才明白：`p + 4` 跳过了全部 4 个元素，指向了数组**后面**的位置。`*p` 读到的不是 `nums[4]`（不存在），而是数组外的任何内存内容。

更可怕的是——编译器不报错。程序可能看起来"正常运行"，但读到的数据完全是随机的。这就是 C 语言"信任程序员"的设计哲学的代价。

当我理解了指针移动的底层规则后，我终于能安全地用它遍历数组、搜索数据，甚至用它写出比索引循环更快的代码。
