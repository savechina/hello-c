...
刚学 C 语言时，我总以为标准库就一个 `printf`。直到有天我需要随机数，四处搜索才发现还有个完整的 `<stdlib.h>`。

当我第一次系统性地浏览标准库时，我震惊了——原来 C 已经内置了这么多工具：字符串操作、数学函数、时间处理、字符分类……它们就像一套瑞士军刀，安静地躺在各种 `<.h>`头文件中，等着我去发现。

这一章不是要你记住每一个函数，而是帮你建立一张**标准库地图**——当你需要某个功能时，知道去哪里找。
...
← [void* 泛型编程](void_generic.md) | [基础阶段复习](review_basic.md) →
