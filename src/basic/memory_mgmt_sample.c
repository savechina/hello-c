...
/* ============================================================
   ...
   ⚠️ NOTE: All heap allocations in this file are strictly
   paired with free(). This file produces ZERO memory leaks
...
static void demo_basic_malloc_free(void) {
    ...
    int32_t *p = malloc(sizeof(int32_t)); // 1. Allocated
    if (p == NULL) {
    ...
    *p = 42;
    ...
    free(p); // 1. Freed (pair for line 23)
    p = NULL;
...
static void demo_malloc_array(void) {
    ...
    size_t n = 5;
    int32_t *arr = malloc(n * sizeof(int32_t)); // 2. Allocated
    if (arr == NULL) {
        fprintf(stderr, "  Error: malloc failed\n");
        return;
    }
    ...
    free(arr); // 2. Freed (pair for line 46)
    arr = NULL;
...
static void demo_calloc_vs_malloc(void) {
    ...
    int32_t *a = malloc(count * sizeof(int32_t)); // 3. Allocated
    if (a == NULL) {
        fprintf(stderr, "  Error: malloc failed\n");
        return;
    }
    ...
    for (size_t i = 0; i < count; i++) {
        /* Values are indeterminate — printing to show garbage */
        printf("%" PRId32 " ", a[i]);
    }
    printf("\n");
    free(a); // 3. Freed (pair for line 76)
    a = NULL;

    printf("  Calloc (zero-initialized):\n");
    int32_t *b = calloc(count, sizeof(int32_t)); // 4. Allocated
    if (b == NULL) {
    ...
    free(b); // 4. Freed (pair for line 91)
    b = NULL;
...
static void demo_realloc_grow(void) {
    ...
    int32_t *buf = malloc(3 * sizeof(int32_t)); // 5. Allocated
    if (buf == NULL) {
        fprintf(stderr, "  Error: malloc failed\n");
        return;
    }
    ...
    if (tmp == NULL) {
        free(buf); // 5. Freed (Original buf on realloc failure)
    ...
    free(buf); // 5. Freed (pair for line 113)
    buf = NULL;
...
static void demo_struct_allocation(void) {
    ...
    Person *p = malloc(sizeof(Person)); // 6. Allocated
    if (p == NULL) {
        fprintf(stderr, "  Error: malloc failed\n");
        return;
    }
    ...
    free(p); // 6. Freed (pair for line 153)
    p = NULL;
    printf("  free(p); p = NULL;\n\n");
}
...
static void demo_memory_leak_concept(void) {
    ...
    char *buf = malloc(256); // 7. Allocated
    if (buf == NULL) {
        fprintf(stderr, "  Error: malloc failed\n");
        return;
    }
    ...
    free(buf); // 7. Freed (pair for line 192)
    buf = NULL;
...
static Person *create_person(const char *name, int32_t age) {
    Person *p = malloc(sizeof(Person)); // 8. Allocated (internal)
    if (p == NULL) {
        return NULL;
    }
...
static void demo_safe_alloc_pattern(void) {
    ...
    free(p); // 8. Freed (pair for line 214, caller responsibility)
    p = NULL;
